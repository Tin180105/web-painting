            ...
            <div class="inner-wrap">
                <div class="inner-content">
                   <span>Danh Mục</span>
                    ...
                    <select name="sort" id="sort" class="inner-arrange-right" onchange="location.href='index.danhmuc.php?sort=' + this.value">
                        <option class="choice" value="">--Chọn--</option>
                        <option value="high" class="inner-high">Sắp xếp theo giá cao</option>
                        <option value="low" class="inner-low">Sắp xếp theo giá thấp</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="col-large-2">
                <div class="inner-wrap">
                    <!-- Cập nhật sản phẩm lên danh mục trên server -->
                        <?php 
                        require('./db/conn.php');
                        // Xác định phương thức sắp xếp từ tham số GET
                        ...
                        // Thêm điều kiện sắp xếp vào câu lệnh SQL nếu có
                        if ($sortOrder === 'high') {
                            $sql_str .= " ORDER BY price DESC"; // Sắp xếp theo giá cao
                        } elseif ($sortOrder === 'low') {
                            $sql_str .= " ORDER BY price ASC"; // Sắp xếp theo giá thấp
                        }
                        // $sql_str = "SELECT products.id as pid, products.name AS pname, images, price, categories.name AS cname, categories.slug AS cslug, brands.slug AS bslug FROM products JOIN categories ON products.category_id = categories.id JOIN brands ON products.brand_id = brands.id";
                        $result = mysqli_query($conn, $sql_str);
                        while ($row = mysqli_fetch_assoc($result)){
